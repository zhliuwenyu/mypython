...
def train(mnist):
    ...
    biases2 = tf.Variable(tf.constant(0.2, shape=[OUTPUT_NODE]))
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAIN_STEPS):
            if(i % 1000 == 0):
                ...
                print("after % step, validation accuracy is %g" % (i, validate_acc))
            xs, ys = mnist.train.next_batch(BATCH_SIZE)
            sess.run(train_op, feed_dict = {x: xs, y_: ys})
        
        test_acc = sess.run(accuracy, feed_dict=test_feed)
        print("after %d training step , accuracy is %g" % (TRAIN_STEPS, test_acc))
